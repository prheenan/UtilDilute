...
def print_dilutions(stock_conc,stock_vol,desired_conc,unit_vol=None,
                    unit_conc=None,**kwargs):
    """
    ...
    """
    sanit = lambda x: np.array(x)
    desired_c = sanit(desired_conc)
    stock_c = sanit(stock_conc)
    stock_v = sanit(stock_vol)
    if (unit_vol is None):
        unit_vol = ["uL" for _ in desired_c]
    if (unit_conc is None):
        unit_conc = ["ng/uL" for _ in desired_c]
    dilutionObj = GetDilutionObjects(stock_c,stock_v,desired_c,**kwargs)
    _PrintVolumeDilutions(dilutionObj,UnitVol=unit_vol,UnitConc=unit_conc)
    return dilutionObj
...
def print_serial_dilution(stock_conc,stock_vol,final_vol,final_conc,
                          string_vol="uL",string_conc="ng/uL",
    ...
    for stock,VolStock,VolDilute,DilutedStock in \
        zip(stock_conc,stock_vol,final_vol,final_conc):
        VolumeTotal = VolStock + VolDilute
        StockStr = "{:5.3g}{:s} of {:5.3g}{:s} with {:5.3g}{:s} {:s}".\
                   format(VolStock,string_vol,stock,string_conc,VolDilute,
                          string_vol,string_buffer)
        ResultStr = "{:5.3g}{:s} of {:5.3g}{:s}".\
                    format(VolumeTotal,string_vol,DilutedStock,string_conc)
...
def PrintSerialSteps(Stock,Volumes,Desired,
    ...
    print_serial_dilution(Stocks,VolumeStock,VolumeDilute,
                          FinalStocks,string_conc=ConcString,
